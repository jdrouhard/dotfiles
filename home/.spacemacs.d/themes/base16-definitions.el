...
(defconst base16-description
  "Color theme by Ethan Schoonover, created 2011-03-24.
Ported to Emacs by Greg Pfeil, http://ethanschoonover.com/solarized.")

(defcustom base16-termcolors 16
  "This is set to 16 by default, meaning that Base16 will attempt to use the
standard 16 colors of your terminal emulator. You will need to set those colors
to the correct Base16 values either manually or by importing one of the many
colorscheme available for popular terminal emulators and Xdefaults."
  :type 'integer
  :options '(16 256)
  :group 'base16)

(defcustom base16-degrade nil
  "For test purposes only; when in GUI mode, forces Base16 to use the 256
degraded color mode to test the approximate color values for accuracy."
  :type 'boolean
  :group 'base16)

(defcustom base16-diff-mode 'normal
  "Sets the level of highlighting to use in diff-like modes."
  :type 'symbol
  :options '(high normal low)
  :group 'base16)

(defcustom base16-bold t
  "Stops Base16 from displaying bold when nil."
  :type 'boolean
  :group 'base16)

(defcustom base16-underline t
  "Stops Base16 from displaying underlines when nil."
  :type 'boolean
  :group 'base16)

(defcustom base16-italic t
  "Stops Base16 from displaying italics when nil."
  :type 'boolean
  :group 'base16)

(defcustom base16-contrast 'normal
  "Stick with normal! It's been carefully tested. Setting this option to high or
low does use the same Base16 palette but simply shifts some values up or
down in order to expand or compress the tonal range displayed."
  :type 'symbol
  :options '(high normal low)
  :group 'base16)

(defcustom base16-broken-srgb
...
\(the default on Macs), Base16 works around it with alternative colors.
However, these colors are not totally portable, so you may be able to edit
the \"Gen RGB\" column in base16-definitions.el to improve them further."
  :type 'boolean
  :group 'base16)
...
(defvar base16-colors           ; ANSI(Base16 terminal)
  '((base00  "#002b36" "#042028" "#1c1c1c" "black"         "black")
    (base01  "#073642" "#0a2832" "#262626" "color-18"      "black")
    (base02  "#586e75" "#465a61" "#585858" "color-19"      "green")
    (base03  "#657b83" "#52676f" "#626262" "brightblack"   "yellow")
    (base04  "#839496" "#708183" "#808080" "color-20"      "blue")
    (base05  "#93a1a1" "#81908f" "#8a8a8a" "white"         "cyan")
    (base06  "#eee8d5" "#e9e2cb" "#e4e4e4" "color-21"      "white")
    (base07  "#fdf6e3" "#fcf4dc" "#ffffd7" "brightwhite"   "white")
    (red     "#dc322f" "#c60007" "#d70000" "red"           "red")
    (orange  "#cb4b16" "#bd3612" "#d75f00" "color-16"      "red")
    ...
    (violet  "#6c71c4" "#5859b7" "#5f5faf" "color-17"      "magenta"))
  "This is a table of all the colors used by the Base16 color theme. Each
   column is a different set, one of which will be chosen oldbasd on term
   capabilities, etc.")

(defun base16-face-for-index (facespec index &optional light)
      ...
      (let ((color-name (plist-get new-fontspec property)))
        (when color-name
          ...
          (when (and (= index 5)
                     (or (and (eq property :background)
                         ...
                         (and (eq property :foreground)
                              (member color-name '(base04 base05)))))
            (setf color-name nil))
          (when (eq color-name 'back)
            (setf color-name 'base00))
          (when light
            (setf color-name
                  (cl-case color-name
                    (base00 'base07)
          ...
          (plist-put new-fontspec
                     property
                     (nth index (assoc color-name base16-colors))))))
    (when (plist-get new-fontspec :box)
      (plist-put new-fontspec
                 :box
                 (base16-face-for-index (plist-get new-fontspec :box) index
                                           light)))
    new-fontspec))

(defun create-face-spec (name facespec)
  `(,name ((((background dark) (type graphic))
            ,(base16-face-for-index facespec
                                       (cond (base16-degrade     3)
                                             (base16-broken-srgb 2)
                                             (t                     1))))
           (((background dark) (type tty) (min-colors 256))
            ,(base16-face-for-index facespec
                                       (if (= base16-termcolors 16) 4 3)))
           (((background dark) (type tty) (min-colors  16))
            ,(base16-face-for-index facespec 4))
           (((background dark) (type tty) (min-colors   8))
            ,(base16-face-for-index facespec 5))
           ...
           (((background light) (type tty) (min-colors  16))
            ,(base16-face-for-index facespec 4 t))
           (((background light) (type tty) (min-colors   8))
            ,(base16-face-for-index facespec 5 t)))))

(defun base16-color-definitions ()
  (let ((bold        (if base16-bold 'bold        'unspecified))
        (bright-bold (if base16-bold 'unspecified 'bold))
        (underline   (if base16-underline t 'unspecified))
        (opt-under   'unspecified)
        (italic      (if base16-italic 'italic 'unspecified)))
    (cond ((eq 'high base16-contrast)
           (let ((orig-base07 base07))
             (rotatef base02 base03 base04 base05 base06 base07)
             (setf base07 orig-base07)))
          ((eq 'low base16-contrast)
           (setf back      base01
                 opt-under t)))
    (let ((bg-back   '(:background back))
          ...
          (bg-base06 '(:background base06))
          (bg-base07 '(:background base07))
          ...
          (fg-base06 '(:foreground base06))
          (fg-base07 '(:foreground base07))
      ...
      (eval-after-load 'ansi-color
        '(setf ansi-color-names-vector [,base01 ,red ,green ,yellow ,blue ,magenta ,cyan ,base00]))
      (mapcar (lambda (face) (apply 'create-face-spec face))
              `(;; basic
                (default (,@fg-base04 ,@bg-back))   ; Normal
                (cursor (,@fg-base00 ,@bg-base04))  ; Cursor
                (shadow (,@fg-base02))
                ...
                (fringe (,@fg-base02 ,@bg-base01))
                (linum (,@fg-base02 ,@bg-base01))
                (header-line (,@fg-base04 ,@bg-base01 ,@fmt-revbb)) ; Pmenu
                (highlight (,@bg-base01))
                (hl-line (:underline ,opt-under ,@bg-base01)) ; CursorLine
                (isearch (,@fmt-stnd ,@fg-orange ,@bg-back)) ; IncSearch
                (isearch-fail (:inherit error)) ; ErrorMsg
                (lazy-highlight (:inherit match)) ; Search
                (menu (,@fg-base04 ,@bg-base01))
                (minibuffer-prompt (,@fmt-bold ,@fg-cyan)) ; Question
                (mode-line ; StatusLine
                 (,@fg-base05 ,@bg-base01 ,@fmt-revbb :box nil))
                (mode-line-inactive    ; StatusLineNC
                 (,@fg-base03 ,@bg-base01 ,@fmt-revbb :box nil))
                (region (,@fg-base02 ,@bg-base00 ,@fmt-revbb)) ; Visual
                (secondary-selection (,@bg-base01))
                (shadow (,@fg-base02))
                (trailing-whitespace (,@fmt-revr ,@fg-red))
                (vertical-border (,@fg-base04))
                ;; comint
                ...
                ;; custom
                (custom-button
                 (,@fg-base05 ,@bg-base01 :box (:line-width 2 :style released-button)))
                (custom-button-mouse
                 (,@fmt-revr ,@fg-base05 ,@bg-base01 :inherit custom-button))
                (custom-button-pressed
                 (,@fmt-revr ,@fg-base05 ,@bg-base01
                             :box (:line-width 2 :style pressed-button)
                             :inherit custom-button-mouse))
                (custom-changed (,@fmt-revr ,@fg-blue ,@bg-base07))
                (custom-comment (,@fg-base05 ,@bg-base01))
                (custom-comment-tag (,@fg-base05 ,@bg-base01))
                (custom-documentation (:inherit default))
                (custom-group-tag (,@fg-base05))
                (custom-group-tag-1 (,@fmt-bold ,@fg-base05))
                (custom-invalid (,@fmt-revr ,@fg-red ,@bg-back))
                (custom-link (,@fg-violet))
                (custom-state (,@fg-green))
                (custom-variable-tag (,@fg-base05))
                ;; diff - DiffAdd, DiffChange, DiffDelete, and DiffText
                ,@(cl-case base16-diff-mode
                    (high
                    ...
                    (normal
                     (if window-system
                         `((diff-added (,@fmt-bold ,@fg-green ,@bg-base01))
                           (diff-changed (,@fmt-bold ,@fg-yellow ,@bg-base01))
                           (diff-removed (,@fmt-bold ,@fg-red ,@bg-base01))
                           (diff-refine-change (,@fmt-bold ,@fg-blue ,@bg-base01)))
                       `((diff-added (,@fg-green ,@bg-base01))
                         (diff-changed (,@fg-yellow ,@bg-base01))
                         (diff-removed (,@fg-red ,@bg-base01))
                         (diff-refine-change (,@fg-blue ,@bg-base01))))))
                ...
                (emacs-wiki-verbatim-face (,@fmt-undr ,@fg-base03))
                ;; eshell
                ...
                (font-lock-doc-face (,@fmt-ital ,@fg-base02)) ; Comment
                ...
                ;; org
                (org-block-background (,@bg-base01))
                (org-hide (,@fg-base00))
                (org-todo (,@fmt-bold ,@fg-base00 ,@bg-red))
                (org-done (,@fmt-bold ,@fg-green))
                (org-todo-kwd-face (,@fmt-bold ,@fg-base00 ,@bg-red))
                (org-done-kwd-face (,@fmt-bold ,@fg-green))
                (org-project-kwd-face (,@fg-violet ,@bg-base00))
                (org-waiting-kwd-face (,@fg-orange ,@bg-base00))
                (org-someday-kwd-face (,@fg-blue ,@bg-base00))
                (org-started-kwd-face (,@fg-yellow ,@bg-base00))
                (org-cancelled-kwd-face (,@fg-green ,@bg-base00))
                (org-delegated-kwd-face (,@fg-cyan ,@bg-base00))
                ...
                (org-headline-done (,@fg-base02))
                (org-priority (,@fmt-ital ,@fg-base02))
                (org-checkbox (,@fmt-bold))
                (org-table (,@fg-cyan))
                (org-formula (:weight bold :slant italic ,@fg-red))
                (org-code (,@fg-base02))
                (org-document-title (,@fmt-bold ,@fg-cyan))
                (org-document-info-keyword (,@fg-base02))
                (org-block (,@fg-base02))
                (org-verbatim (,@fmt-undr ,@fg-base02))
                (org-clock-overlay (,@fmt-revr ,@bg-cyan ,@fg-base00))
                ...
                (col-highlight (,@bg-base01))
                ;; ace-jump-mode
                (ace-jump-face-background (,@fmt-none ,@fg-base02))
                (ace-jump-face-foreground (,@fmt-bold ,@fg-red))
                ;; bm visual bookmarks
                (bm-fringe-face (,@bg-orange ,@fg-base00))
                (bm-fringe-persistent-face (,@bg-blue ,@fg-base00))
                ;; Flymake
                (flymake-errline (,@fmt-bold ,@fg-red)) ; Error
                (flymake-warnline (,@fmt-bold ,@fg-red))
                ;; column-marker
                (column-marker-1 (,@bg-base02))
                ...
                (jabber-chat-text-foreign (,@fg-base05))
                (jabber-chat-text-local (,@fg-base04))
                ...
                (jabber-roster-user-offline (,@fg-base02))
                ...
                (gnus-cite-11 (,@fg-base01))
                (gnus-group-mail-1 (,@fmt-bold ,@fg-base07))
                (gnus-group-mail-1-empty (,@fg-base07))
                (gnus-group-mail-2 (,@fmt-bold ,@fg-base06))
                (gnus-group-mail-2-empty (,@fg-base06))
                (gnus-group-mail-3 (,@fmt-bold ,@fg-magenta))
                (gnus-group-mail-3-empty (,@fg-magenta))
                (gnus-group-mail-low (,@fmt-bold ,@fg-base03))
                (gnus-group-mail-low-empty (,@fg-base03))
                (gnus-group-news-1 (,@fmt-bold ,@fg-base05))
                (gnus-group-news-1-empty (,@fg-base05))
                ...
                (gnus-emphasis-highlight-words ; highlight
                 (,@fmt-none ,fg-yellow))
                (gnus-header-content (,@fmt-none ,@fg-base02)) ; hdrdefault
                (gnus-header-from (,@fmt-none ,@fg-base03)) ; header ^From
                (gnus-header-name (,@fmt-none ,@fg-base02)) ; hdrdefault
                (gnus-header-newsgroups (,@fmt-none ,@fg-base01)) ; hdrdefault
                (gnus-header-subject (,@fmt-none ,@fg-blue)) ; header ^Subject
                (gnus-server-agent (,@fmt-bold ,@fg-base07))
                (gnus-server-closed (,@fmt-ital ,@fg-base05))
                (gnus-server-denied (,@fmt-bold ,@fg-base06))
                (gnus-server-offline (,@fmt-bold ,@fg-green))
                (gnus-server-opened (,@fmt-bold ,@fg-cyan))
                (gnus-signature (,@fmt-none ,@fg-base02)) ; signature
                (gnus-splash (,@fg-base06))
                ...
                (gnus-summary-normal-undownloaded (,@fmt-none ,@fg-base06))
                (gnus-summary-normal-unread ; unread messages
                 (,@fmt-none ,@fg-blue))
                (gnus-summary-selected ; indicator
                 (,@fmt-none ,@fg-base00 ,@bg-yellow))
                ;;helm
                (helm-apt-deinstalled (,@fg-base02))
                ...
                (helm-buffer-size (,@fg-base02))
                (helm-candidate-number (,@fmt-bold ,@bg-base01 ,@fg-base05))
                (helm-emms-playlist (,@fmt-none ,@fg-base02))
                ...
                (helm-ff-invalid-symlink (,@bg-base01 ,@fg-red))
                ...
                (helm-lisp-completion-info (,@fg-base04))
                (helm-lisp-show-completion (,@fmt-bold ,@fg-yellow ,@bg-base01))
                (helm-ls-git-added-copied-face (,@fg-green))
                (helm-ls-git-conflict-face (,@fmt-bold ,@fg-red))
                (helm-ls-git-deleted-and-staged-face (,@fmt-ital ,@fg-base02))
                (helm-ls-git-deleted-not-staged-face (,@fmt-bold ,@fg-green))
                (helm-ls-git-modified-and-staged-face (,@fmt-ital ,@fg-base02))
                (helm-ls-git-modified-not-staged-face (,@fmt-ital ,@fg-base02))
                ...
                ;; Message
                (message-mml (,@fg-blue))
                (message-cited-text (,@fg-base06))
                (message-separator (,@fg-base07))
                ...
                (message-header-subject (,@fg-base03))
                (message-header-cc (,@fmt-bold ,@fg-green))
                (message-header-to (,@fmt-bold ,@fg-base05))
                ;; minimap
                (minimap-active-region-background (,@bg-base01))
                (minimap-semantic-function-face (,bg-base07))
                (minimap-semantic-type-face (,bg-base07))
                (minimap-semantic-variable-face (,bg-base07))
                ;; parenface
                ...
                ;; powerline
                (powerline-active1 (,@fg-base03 :inherit mode-line))
                (powerline-active2 (,@fg-base04 :inherit mode-line))
                (powerline-inactive1 (,@fg-base01 ,@bg-base05 :inherit mode-line-inactive))
                (powerline-inactive2 (,@fg-base02 :inherit mode-line-inactive))
                ;; slime
                (slime-error-face (,@fmt-revr ,@fg-red)) ; ErrorMsg
                (slime-note-face (,@fg-yellow))
                (slime-repl-inputted-output-face (,@fg-red))
                (slime-repl-output-mouseover-face (:box (:color base07)))
                (slime-style-warning-face (,@fmt-bold ,@fg-orange))
                (slime-warning-face (,@fmt-bold ,@fg-red)) ; WarningMsg
                ;; smartparens
                (sp-pair-overlay-face (,@bg-base01))
                (sp-wrap-overlay-face (,@bg-base01))
                (sp-wrap-tag-overlay-face (,@bg-base01))
                (sp-show-pair-match-face (,@fg-magenta ,@bg-back))
                (sp-show-pair-mismatch-face (,@bg-red ,@fg-base01))
                ;; whitespace
                (whitespace-empty (,@fg-red))
                (whitespace-hspace (,@fg-orange))
                (whitespace-indentation (,@fg-base01))
                (whitespace-space (,@fg-base01))
                (whitespace-space-after-tab (,@fg-cyan))
                (whitespace-space-before-tab (,@fmt-bold ,@fg-red))
                (whitespace-tab (,@fg-base01))
                (whitespace-trailing (,@fmt-bold ,@fg-red ,@bg-base01))
                ...
                (rcirc-server (,@fg-base05))
                (rcirc-timestamp (,@fg-base02))
                ;;font-latex
                ...
                (term-color-white (,@fg-base03))
                ;; company
                (company-tooltip (,@fg-base03 ,@bg-base01))
                (company-tooltip-selection (,@fg-green ,@bg-base01))
                (company-tooltip-mouse (,@fg-base05 ,@bg-base01))
                (company-tooltip-common (,@fg-blue ,@bg-base01 ,@fmt-undr))
                (company-tooltip-common-selection (,@fg-green ,@bg-base01 ,@fmt-undr))
                (company-tooltip-annotation (,@fg-yellow ,@bg-base01))
                (company-scrollbar-fg (,@bg-base04))
                (company-scrollbar-bg (,@bg-base01))
                (company-preview (,@bg-green))
                (company-preview-common (,@bg-base01))
                (company-template-field (,@fg-base00 ,@bg-yellow))
                ;; hydra
...
(defmacro create-base16-theme (name description color-definitions)
  `(progn
