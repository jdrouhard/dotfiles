...
function! AlternateInline(bang)
   let alt = expand('%:p:s,_inline.h,.XINLINEX,:s,.h$,_inline.h,:s,.XINLINEX$,.h,')
   let found = findfile(alt)
   if (found != "")
      call <SID>FindOrCreateBuffer(found, "n".a:bang, 1)
   endif
endfunction
...
comm! -nargs=? -bang AI call AlternateInline("<bang>")
